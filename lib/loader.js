...
exports.getConfig = function ( file ) {
    ...
    for ( var key in paths ) {
        ...
        if ( edp.path.isRelativePath( pathValue ) ) {
            pathValue = edp.path.relative( '/' + baseUrl, '/' + pathValue );
        }
    ...
    moduleConf.packages.forEach( function ( pkg ) {
        ...
        if ( edp.path.isRelativePath( pkgLocation ) ) {
            pkgLocation = edp.path.relative( '/' + baseUrl, '/' + pkgLocation );
        }
        ...
        for (var j = 0; j < oldPackages.length; j ++) {
            ...
            if ( oldPkg.name === newPkg.name ) {
                ...
                if ( oldPkg.location !== newPkg.location ||
                    ...
                    // 如果出现不一致的情况，直接替换
                    edp.log.info("Replace [%s] with [%s] @ [%s]",
                        oldPkg.location, newPkg.location, file);
